...
CI3_TIMEOUT    = 1200        # Seconds before aborting if Conveyor 1 end sensor (CI3) is not triggered
PLC_TIMEOUT    = 1200        # Seconds before aborting if PLC lid confirmation (ACK) is not received

# Maximum number of cakes to palletize before stopping (single 2x2 floor, no separator lids)
MAX_PALLET_CAKES = 4
...
class RobotMain(object):
    ...
    def _paletizar_pastel(self, flavor):
        ...
        Only floor 1 is used (4 cakes total). No separator lids are placed."""
        grid_idx           = self.global_pallet_count % 4
        target_x, target_y = self.PALLET_POSITIONS[grid_idx]
        target_z           = self.PALLET_Z_PISO1

        print(f"[STEP G] Cake {self.global_pallet_count+1}/{MAX_PALLET_CAKES} → "
              f"Slot {grid_idx+1}/4, Floor 1, "
    ...
    def run(self):
        ...
        try:
            print("System ready. Starting pipelining loop (max. "
                  f"{MAX_CONCURRENT} concurrent cakes, target: {MAX_PALLET_CAKES} cakes)...")

            while self.is_alive:
                # 1. PRE-ITERATION: Purge completed tasks to free slots
                self._purge_completed()

                # STOP CONDITION: All 4 pallet slots are filled and the pipeline is empty.
                # Tasks already in flight are allowed to complete normally before stopping.
                if self.global_pallet_count >= MAX_PALLET_CAKES and self._active_count() == 0:
                    print(f"[DONE] All {MAX_PALLET_CAKES} pallet slots filled. Stopping.")
                    break
                ...
                # Guard: do not accept new orders once the pallet target is reached or
                # already has enough tasks in-flight to fill the remaining slots.
                tasks_in_flight = self._active_count()
                slots_remaining = MAX_PALLET_CAKES - self.global_pallet_count
                if not action_taken and tasks_in_flight < MAX_CONCURRENT and tasks_in_flight < slots_remaining:
                    ...
                    if flavor is not None:
                        ...
                        if not already_in_progress:
                            print(f"\n[P4] New task: {flavor} col={idx+1}, pallet={self.global_pallet_count+1}/{MAX_PALLET_CAKES}")
